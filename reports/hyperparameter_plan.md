...
XGBoost runtime device: `cuda` (`hist` tree method). An explicit `cuda` configuration fails fast unless `nvidia-smi` confirms an available NVIDIA GPU; automatic CPU selection occurs only when `xgboost_device=auto`.
